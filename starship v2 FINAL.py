...
SKINS = [
    ...
    {"name": "Emerald", "unlock": 40, "color": (0, 255, 100), "accent": (100, 255, 100), "laser_color": (0, 200, 80), "booster_color": (0, 150, 50)},
...
# ========== AUTO-AIM LASER CLASS (FIXED - Now with trail effect) ==========
class AutoAimLaser:
    ...
    def draw(self, screen):
        ...
        for i, pos in enumerate(self.trail):
            ...
            trail_color = (min(255, self.color[0] + 100), self.color[1], self.color[2])
            pygame.draw.circle(screen, trail_color, (int(pos[0]), int(pos[1])), 3)
...
class Player:
    ...
    def update(self, keys):
        ...
        # Store previous position for booster effect
    ...
    def add_booster_particle(self, prev_pos, direction):
        """Add a booster particle effect in the opposite direction of movement"""
        # Only add particles if moving
        if prev_pos[0] == self.rect.centerx and prev_pos[1] == self.rect.centery:
            return
        
        # Calculate particle position (behind the ship)
        if direction == 'left':
    ...
    def shoot_auto_aim(self, enemies, bosses):
        """Special auto-aim shot for Iron Spider mode"""
        ...
        if self.laser_cooldown == 0:
            ...
            for dist, enemy, boss in targets:
                if enemy or boss:
                    ...
                    self.lasers.append(AutoAimLaser(laser_x, laser_y, enemy, boss, self.skin["color"]))
                    lasers_created += 1
            
            # If no targets found, shoot in spread pattern
            if lasers_created == 0:
    ...
    def draw(self, screen):
        ...
        for particle in self.booster_particles:
            ...
            color = (self.skin["booster_color"][0], 
                    self.skin["booster_color"][1], 
                    self.skin["booster_color"][2])
        ...
        # Draw glowing effect for powerups
        if self.powerups["godmode"]:
            for i in range(3):
        ...
        if not self.powerups["godmode"]:
            if self.is_reloading:
                ...
                pygame.draw.rect(screen, (100, 0, 0), (60, 95, bar_width, bar_height))
                pygame.draw.rect(screen, RED, (60, 95, progress_width, bar_height))
            else:
                ...
                pygame.draw.rect(screen, (50, 50, 50), (60, 95, bar_width, bar_height))
                pygame.draw.rect(screen, ammo_color, (60, 95, ammo_width, bar_height))
        else:
...
class Laser:
    ...
    def draw(self, screen):
        ...
        pygame.draw.rect(screen, self.color, self.rect)
        # Red tip for ammo
        pygame.draw.rect(screen, RED, (self.rect.x, self.rect.y, self.rect.width, 3))
...
class PowerUp:
    def __init__(self, x=None, y=None):
        # Lowered spawn rates: removed duplicates, each appears less frequently
        powerup_types = ["rapid_fire", "shield", "triple_shot", "heal"]  # godmode removed from regular spawns
...
def carousel_menu():
    ...
    while True:
        ...
        preview_ship = pygame.Surface((100, 80), pygame.SRCALPHA)
        pygame.draw.polygon(preview_ship, s["color"], [(50, 0), (0, 80), (100, 80)])
        pygame.draw.polygon(preview_ship, s["accent"], [(10, 40), (0, 80), (30, 70)])
        pygame.draw.polygon(preview_ship, s["accent"], [(90, 40), (100, 80), (70, 70)])
        pygame.draw.circle(preview_ship, CYAN, (50, 30), 10)
...
def main_game():
    ...
    powerup_spawn_timer = random.randint(500, 800)  # Increased spawn delay for powerups
    godmode_spawn_timer = random.randint(1800, 3600)  # Rare godmode spawn timer
    level = 1
    game_over = False
    paused = False
    auto_fire_timer = 0
    ...
    while not game_over:
        ...
        # Update enemies - FIXED: Using proper iteration to avoid list modification issues
            ...
            for laser in player.lasers[:]:
                if enemy.rect.colliderect(laser.rect):
                    ...
                    if enemy.health <= 0:
                        ...
                        if enemy in enemies:  # Check if still in list before removing
                            enemies.remove(enemy)
                        if 'explosion_sound' in globals():
                            explosion_sound.play()
                        if random.random() < 0.15:  # Slightly lower powerup drop chance
